...
#pragma pack(pop)
    ...
    inline bool CreateDriverHandle() {
        cache::driverHandle = CreateFile(L"\\\\.\\PicoDriver",
        ...
        cache::driverHandle = cache::driverHandle;
        return true;
    }

    inline DWORD GetProcessIdByName(const wchar_t* processName) {
        if (!cache::driverHandle || processName == nullptr) {
            return 0;
        }
    ...
    {
        CreateDriverHandle();
        cache::processId = GetProcessIdByName(processName);
        Request r;

        r.process_id = reinterpret_cast<HANDLE>(static_cast<UINT_PTR>(cache::processId));

        return DeviceIoControl(cache::driverHandle, attach, &r, sizeof(r), &r, sizeof(r), nullptr, nullptr);
    }

    inline uintptr_t GetModuleBaseByName(const wchar_t* moduleName) {
        ModuleRequest request = {};
        request.process_id = reinterpret_cast<HANDLE>(cache::processId);
    ...
    __forceinline T Read(const std::uintptr_t addr) {
        alignas(T) T temp {};
        Request r{
            .process_id = reinterpret_cast<HANDLE>(cache::processId),
            .target = reinterpret_cast<PVOID>(addr),
            .buffer = &temp,
            .size = sizeof(T)
        };

        DeviceIoControl(cache::driverHandle, read, &r, sizeof(r),
    ...
    __forceinline void Write(const std::uintptr_t addr, const T& value) {
        ...
        DeviceIoControl(cache::driverHandle, write, &r, sizeof(r),
            &r, sizeof(r), nullptr, nullptr);
    }
    ...
    inline static std::string ReadString(const std::uintptr_t addr, size_t maxLength = 256) {
        ...
        if (maxLength <= STACK_BUFFER_SIZE) {
            ...
            if (DeviceIoControl(cache::driverHandle, read, &r, sizeof(r),
                &r, sizeof(r), nullptr, nullptr)) {
    ...
    inline bool MoveMouse(long x, long y, unsigned short flags = MOUSE_MOVE_ABSOLUTE) {
        ...
        if (!cache::driverHandle) return false;
